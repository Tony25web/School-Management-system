...
class ClassLevel {
    ...
    static async fetchClassLevels(req, res, next) {
        try {
            ...
            if (!classLevels) {
                throw new APIError_1.APIError("there is no Class Level created yet", http_status_codes_1.StatusCodes.NOT_FOUND);
            }
    ...
    static async fetchAClasslevel(req, res, next) {
        try {
            ...
            if (!classLevel) {
                throw new APIError_1.APIError("there is no Class Level with the specified id found ", http_status_codes_1.StatusCodes.NOT_FOUND);
            }
            res.status(http_status_codes_1.StatusCodes.OK).json({
    ...
    static async updateAClassLevel(req, res, next) {
        try {
            ...
            if (!classLevel) {
                throw new APIError_1.APIError("could not update the Class because there is no record with the specified id found ", http_status_codes_1.StatusCodes.NOT_FOUND);
            }
            res.status(http_status_codes_1.StatusCodes.OK).json({
                message: "A Class was successfully retrieved",
                classLevel,
    ...
    static async deleteAClassLevel(req, res, next) {
        try {
            ...
            if (!classLevel) {
                throw new APIError_1.APIError("there is no Class with the specified id try again", http_status_codes_1.StatusCodes.NOT_FOUND);
            }
            res
